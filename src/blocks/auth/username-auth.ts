...
// ponytail: stdlib first, no new dep for what scrypt does.
//
// Password is OPTIONAL at registration:
//   - With password: account is password-protected (production-grade)
//   - Without password: username-only (trusted environments only — anyone who
//     knows the username can log in. Documented limitation.)
//
// The DB column `password_hash` is NULL for username-only accounts, non-NULL
// for password-protected accounts. The login flow checks accordingly.
...
export function registerUser(input: RegisterInput): OpResult<{ user_id: string; username: string; token: string }> {
  ...
  // Password validation (optional but if provided must be strong enough)
  let passwordHash: string | null = null;
  if (input.password !== undefined && input.password !== '') {
    if (input.password.length < 8) {
      return err('WEAK_PASSWORD', 'Password must be at least 8 characters', 400);
    }
    if (input.password.length > 1000) {
      return err('PASSWORD_TOO_LONG', 'Password must be 1000 characters or less', 400);
    }
...
export function loginUser(input: { username: string; password?: string }): OpResult<{ user_id: string; username: string; token: string }> {
  ...
  if (user.password_hash) {
    if (!input.password) return err('PASSWORD_REQUIRED', 'Password required for this account', 401);
    if (!verifyPassword(input.password, user.password_hash)) {
      return err('INVALID_CREDENTIALS', 'Invalid username or password', 401);
    }
  }
  // else: username-only account — no password needed (trusted environments only)
